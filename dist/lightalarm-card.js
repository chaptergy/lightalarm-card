...
var config = {
	...
	required_entity_missing: "A required entity %entity% is not set",
	required_entity_not_found: "A required entity %entity% is not found",
	force_native_time_picker_for_device: "Force the native time picker in this browser"
};
var card = {
...
let LightalarmCardEditor = class LightalarmCardEditor extends LitElement {
    setConfig(config) {
        this._config = config;
    }
    get _title() {
    ...
    get _duration_entity() {
        if (this._config) {
            return this._config.duration_entity || '';
        }
        return '';
    }
    get _force_native_time_picker_for_device() {
...
], LightalarmCardEditor.prototype, "hass", void 0);
__decorate([
    property({ attribute: false })
], LightalarmCardEditor.prototype, "_config", void 0);
LightalarmCardEditor = __decorate([
    customElement('lightalarm-card-editor')
], LightalarmCardEditor);

const CARD_VERSION = '3.0.0';

/* eslint no-console: 0 */
console.info(`%c LIGHTALARM-CARD %c ${CARD_VERSION} `, 'color: cornsilk; font-weight: bold; background: firebrick', 'color: firebrick; font-weight: bold; background: cornsilk');
let LightalarmCard = class LightalarmCard extends LitElement {
    constructor() {
        super(...arguments);
        this.timeInputStatus = 'none';
    }
    static async getConfigElement() {
        return document.createElement('lightalarm-card-editor');
    }
    ...
    shouldUpdate(changedProps) {
        const shouldUpdate = Y(this, changedProps, false);
        // Only check if it doesn't update anyway
        if (!shouldUpdate) {
            const changedHass = changedProps['hass'];
            return (!changedHass || // hass is not set
        ...
        <div class="lightalarm-wrapper" id="lightalarm-wrapper">
          ...
          <div class="alarm-properties-wrap">
            ...
            >
              ...
              >
                ${repeat(modeStateObj.attributes.options, option => html `
    ...
    _showTimeInputs() {
        ...
        if (!timeInputWrap || this.timeInputStatus === 'shown')
            return;
     ...
     */
    _selectedTimeValueChangedPicker(ev) {
        ...
        if (localStorage.getItem('lightalarmCard.forceNativePicker') === 'true') {
            ...
            const hourInput = this.shadowRoot.getElementById('lightalarm-time-input-hour');
            hourInput.value = newTime.split(':')[0];
            hourInput === null || hourInput === void 0 ? void 0 : hourInput.blur();
            clearTimeout(this.inputBlurTimer);
            const minuteInput = this.shadowRoot.getElementById('lightalarm-time-input-minute');
            minuteInput.value = newTime.split(':')[1];
            minuteInput === null || minuteInput === void 0 ? void 0 : minuteInput.blur();
            clearTimeout(this.inputBlurTimer);
            const timeInputWrap = this.shadowRoot.getElementById('lightalarm-wrapper');
            if (!timeInputWrap)
                return;
    ...
    _selectedTimeValueChangedInput() {
        this._debounce('saveTimeInputValue', 3000);
    }
    _saveTimePickerValue(newTime) {
        ...
        const leftPicker = this.shadowRoot.getElementById('lightalarm-time-picker-left');
        leftPicker.value = newTime;
        const rightPicker = this.shadowRoot.getElementById('lightalarm-time-picker-right');
    ...
    _saveTimeInputValue() {
        const stateObj = this.hass.states[this.config.time_entity];
        const timeInputHour = this.shadowRoot.getElementById('lightalarm-time-input-hour');
        const timeInputMinute = this.shadowRoot.getElementById('lightalarm-time-input-minute');
        const timeInputHourValue = Number(timeInputHour.value);
        const timeInputMinuteValue = Number(timeInputMinute.value);
        // Cancel if values invalid
        if (isNaN(timeInputHourValue) ||
            isNaN(timeInputMinuteValue) ||
            timeInputHourValue < 0 ||
            timeInputHourValue > 23 ||
            timeInputMinuteValue < 0 ||
            timeInputMinuteValue > 59) {
            console.error(`Values for lightalarm time of '${timeInputHour.value}' hours and '${timeInputMinute.value}' is not a valid time!`);
            return;
        }
    ...
    _timePickerClick(focusHours) {
        if (localStorage.getItem('lightalarmCard.forceNativePicker') === 'true') {
            ...
            clearTimeout(this.inputBlurTimer);
            if (focusHours) {
                const input = this.shadowRoot.getElementById('lightalarm-time-input-hour');
                input === null || input === void 0 ? void 0 : input.focus();
                this._moveCursorToEnd(input);
            }
            else {
                const input = this.shadowRoot.getElementById('lightalarm-time-input-minute');
                input === null || input === void 0 ? void 0 : input.focus();
                this._moveCursorToEnd(input);
            }
        }
    ...
    _selectedDurationValueChanged() {
        const durationInputEl = this.shadowRoot.querySelector('#duration-input');
    ...
    _debounce(func, delay, instant = false, newTime = '') {
        clearTimeout(this.debounceTimer);
        switch (func) {
            case 'saveTimeInputValue':
                if (instant)
                    this._saveTimeInputValue();
                else
                    this.debounceTimer = setTimeout(() => this._saveTimeInputValue(), delay);
                break;
            case 'saveTimePickerValue':
                if (instant)
                    this._saveTimePickerValue(newTime);
                else
                    this.debounceTimer = setTimeout(() => this._saveTimePickerValue(newTime), delay);
                break;
        }
    }
};
__decorate([
    property()
], LightalarmCard.prototype, "hass", void 0);
__decorate([
    property()
], LightalarmCard.prototype, "config", void 0);
LightalarmCard = __decorate([
    customElement('lightalarm-card')
], LightalarmCard);

export { LightalarmCard };
